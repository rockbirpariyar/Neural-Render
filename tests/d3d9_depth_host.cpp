...
int wmain(int argc, wchar_t **argv)
{
    ...
    const auto present_scene = [&](unsigned phase)
    {
        ...
        if (!succeeded(objects.device->Present(nullptr, nullptr, nullptr, nullptr), "Present") ||
    ...
    if (!succeeded(objects.device->Reset(&parameters), "Reset(with acquired depth)") ||
    ...
    if (after_reset_log.find("Depth samples: valid=256/256") == std::string::npos || !check_depth_log(log_path))
    {
