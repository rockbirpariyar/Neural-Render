...
namespace
{
    ...
    std::uint64_t motion_frame_count = 0, motion_failure_count = 0;
    ...
    bool motion_debug = false, motion_active_logged = false, motion_error_logged = false;
    ...
    bool open_log_and_config(HMODULE addon_module)
    {
        ...
        motion_debug = GetPrivateProfileIntW(L"ENR", L"MotionDebug", 0, path) != 0;
    ...
    void log_completed_frames()
    {
        ...
        const int length = std::snprintf(message, sizeof(message),
            ...
            static_cast<unsigned long long>(motion_frame_count),
            static_cast<unsigned long long>(motion_failure_count));
        if (length > 0 && static_cast<std::size_t>(length) < sizeof(message))
        {
            DWORD written = 0;
    ...
    void log_temporal_result(const enr::temporal_report &report)
    {
        ...
        if (report.motion_resources_initialized)
        {
            write_line("Motion resources initialized");
            write_formatted("Motion field: %ux%u", report.motion_width, report.motion_height);
            motion_error_logged = false;
        }
        if (report.motion_processed)
        {
            ++motion_frame_count;
            if (!motion_active_logged)
            {
                write_line("Motion estimation ACTIVE");
                motion_active_logged = true;
            }
        }
        if (FAILED(report.motion_result))
        {
            ++motion_failure_count;
            if (!motion_error_logged)
            {
                write_formatted("Motion estimation unavailable: HRESULT 0x%08X", static_cast<unsigned>(report.motion_result));
                motion_error_logged = true;
            }
        }
    }
    ...
    HRESULT draw_frame(reshade::api::effect_runtime *runtime)
    {
        ...
        if (runtime->is_key_down(VK_CONTROL))
        {
            ...
            if (runtime->is_key_pressed(VK_F10))
            {
                ...
                write_line(motion_debug ? "Motion visualization enabled" : "Motion visualization disabled");
            }
        ...
        if (SUCCEEDED(result))
        {
            const enr::temporal_view view = motion_debug ? enr::temporal_view::motion :
                history_debug == 1 ? enr::temporal_view::previous_color :
...
extern "C"
{
    ...
    __declspec(dllexport) bool AddonInit(HMODULE addon_module, HMODULE reshade_module)
    {
        ...
        if (motion_debug) write_line("Motion visualization enabled");
