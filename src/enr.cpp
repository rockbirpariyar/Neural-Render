...
namespace
{
    ...
    HRESULT draw_frame(reshade::api::effect_runtime *runtime)
    {
        ...
        if (grayscale == nullptr || device == nullptr ||
            device->get_api() != reshade::api::device_api::d3d9) return E_NOINTERFACE;
        ...
        if (native_device == nullptr || native_swapchain == nullptr) return E_POINTER;
        ...
        if (SUCCEEDED(result))
        {
            ...
            if (depth_debug && source.texture != nullptr)
                result = grayscale->render_depth(native_device, backbuffer, source.texture,
                    depth_linearize, depth_reversed, initialized_now);
            else
                result = grayscale->render(native_device, backbuffer, initialized_now);
        }
    ...
    void on_destroy_effect_runtime(reshade::api::effect_runtime *) { release_resources(); }
    void on_destroy_swapchain(reshade::api::swapchain *, bool) { release_resources(); }
    void on_destroy_device(reshade::api::device *) { release_resources(); }
}
